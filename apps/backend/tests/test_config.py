...
class TestGenerateConfigFiles(unittest.TestCase):
    ...
    single_float_param_hyper_param: Dict[str, Parameter] = {"x": float_param}
    ...
    single_bool_param_hyper_param: Dict[str, Parameter] = {"b": bool_param}
    ...
    def test_single_float_variable(self):
        self.reset()
        self.exp_info.hyperparameters = self.single_float_param_hyper_param
        generate_config_files(self.exp_info)
        configs = self.exp_info.configs
        self.assertEqual(len(configs), 5)
        self.assertConfigKeys(5, configs)
    ...
    def test_single_bool_variable(self):
        self.reset()
        self.exp_info.hyperparameters = self.single_bool_param_hyper_param
        generate_config_files(self.exp_info)
        configs = self.exp_info.configs
        self.assertEqual(len(configs), 2)
        self.assertConfigKeys(2, configs)
